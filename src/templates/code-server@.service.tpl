...
ExecStartPre=/bin/bash -c 'home=$(getent passwd %i | cut -d: -f6); uid=$(id -u %i); port=$((20000 + uid)); mkdir -p "$home/.config/code-server"; echo "bind-addr: 127.0.0.1:$port" > "$home/.config/code-server/config.yaml"; echo "auth: none" >> "$home/.config/code-server/config.yaml"'
